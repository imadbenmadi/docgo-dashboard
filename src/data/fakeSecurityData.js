const generateFakeSecurityData = () => {
  ...
  const adminEmails = [
    "admin@healthpathglobal.com",
    "john.doe@healthpathglobal.com",
    "sarah.smith@healthpathglobal.com",
    "mike.wilson@healthpathglobal.com",
    "lisa.johnson@healthpathglobal.com",
    "david.brown@healthpathglobal.com",
  ];
  ...
  const generateLoginAttempt = (id) => {
    ...
    return {
      ...
      admin: {
        email: adminEmails[Math.floor(Math.random() * adminEmails.length)],
      },
      loginStatus: status,
      failureReason:
        status !== "SUCCESS"
      ...
      threatReasons: isThreat
        ? [
            threatReasons[Math.floor(Math.random() * threatReasons.length)],
            threatReasons[Math.floor(Math.random() * threatReasons.length)],
  ...
  const generateLogins = (count = 50) => {
    ...
    return logins.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // Sort by most recent first
  };
  ...
  const generateStats = (logins) => {
    ...
    const threatenedLogins = logins.filter((login) => login.isThreat).length;
  ...
  const generateCompleteSecurityData = (options = {}) => {
    ...
    return {
      ...
      pagination: generatePagination(allLogins.length, currentPage, limit),
    };
  };
