...
const CourseCard = ({ course, handleView, handleEdit }) => {
  ...
  const enrolledCount =
    course.stats?.enrolledCount ?? course.Users_count ?? approvedApplications;
    ...
    <div className="bg-white mx-auto rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all transform hover:scale-105 border border-gray-100 h-[32rem] flex flex-col w-80">
      <div className="relative h-48">
        ...
        <div
          ...
          style={{
            display: course.Image || course.coverImage ? "none" : "flex",
          }}
        >
          <svg
            ...
            <span className="text-xl font-bold text-gray-800">
              {hasDiscount
                ? parseFloat(course.discountPrice) === 0
                  ? "Gratuit"
                  : `${parseFloat(course.discountPrice).toFixed(2)}DZD`
                : parseFloat(course.Price || 0) === 0
                  ? "Gratuit"
                  : `${parseFloat(course.Price || 0).toFixed(2)}DZD`}
            </span>
          </div>
          <div className="text-sm text-gray-500">
            <span className="text-xs">Inscrits: {enrolledCount}</span>
          </div>
        </div>
