...
const CourseDetailModal = ({ course, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto overflow-x-auto">
        {/* Header */}
        ...
        <div className="flex gap-3 my-6 pb-6 border-b border-gray-200 mx-12">
          {course.isDeleted ? (
            <div className="flex-1 bg-gray-100 text-gray-500 w-fit px-6 py-3 rounded-lg font-semibold flex items-center justify-center gap-2 border border-gray-200">
              <X className="w-5 h-5" />
              Cours supprimé, accès désactivé
            </div>
          ) : (
            <button
              onClick={() =>
                (window.location.href = `/Courses/${course.id || course.ID}`)
              }
        ...
        <div className="p-6">
          ...
          {/* Short Descriptions */}
          {(course.shortDescription || course.shortDescription_ar) && (
            <div>
              <h4 className="font-semibold text-gray-800">Résumé</h4>
