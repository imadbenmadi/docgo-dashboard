  ...
  const fetchOptions = useCallback(async () => {
    try {
      ...
      const uniqueCategories = [
        ...new Set(
          faqs.map((faq) => faq.category).filter((category) => category),
        ),
      ];
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Manager les questions fréquentes pour la page d'accueil, les cours
            et les programmes.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="text-2xl font-bold text-purple-600">
            {faqs.filter((faq) => faq.assignmentType === "course").length}
          </div>
          <div className="text-gray-600">Course FAQs</div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="text-2xl font-bold text-indigo-600">
            {faqs.filter((faq) => faq.assignmentType === "program").length}
          </div>
                    ...
                    <tr key={faq.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-xs text-gray-500">
                          <div>Views: {faq.viewCount}</div>
                          <div>Helpful: {faq.helpfulCount}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <p className="text-sm text-gray-700">
                      Showing page {currentPage} of {totalPages} ({totalFaqs}{" "}
                      total FAQs)
                    </p>
                  </div>
                  <div className="flex space-x-2">
