...
const UserAnalytics = () => {
    ...
    datasets: [
      {
        ...
        data:
          data?.usersByCountry?.slice(0, 10).map((item) => item.count) || [],
        backgroundColor: [
          "rgba(59, 130, 246, 0.8)",
          "rgba(16, 185, 129, 0.8)",
          "rgba(245, 158, 11, 0.8)",
  ...
  // Prepare chart data for study domains
  const studyDomainChartData = {
    labels:
    ...
    datasets: [
      {
        data:
          data?.usersByStudyDomain?.slice(0, 8).map((item) => item.count) || [],
        backgroundColor: [
          "rgba(239, 68, 68, 0.8)",
          "rgba(245, 158, 11, 0.8)",
          "rgba(34, 197, 94, 0.8)",
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* Study Domains Distribution */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Study Domains Distribution
          </h3>
          <div className="h-80">
            <Doughnut data={studyDomainChartData} options={doughnutOptions} />
          </div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.usersByCountry?.slice(0, 8).map((item, index) => {
                  const percentage =
                    data.totalUsers > 0
                      ? (item.count / data.totalUsers) * 100
                      : 0;
                  return (
                    <tr key={index}>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.usersByStudyDomain?.slice(0, 8).map((item, index) => {
                  const percentage =
                    data.totalUsers > 0
                      ? (item.count / data.totalUsers) * 100
                      : 0;
                  return (
                    <tr key={index}>
                      <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {item.studyDomain || "Not specified"}
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.count.toLocaleString()}
                      </td>
