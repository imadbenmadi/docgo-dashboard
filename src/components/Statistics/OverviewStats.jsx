        ...
        {items?.slice(0, 5).map((item, index) => (
          <div key={index} className="flex items-center justify-between">
            <span className="text-sm text-gray-600 truncate max-w-xs">
              {item.page || item.referrer || item.country || "Unknown"}
            </span>
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
        <TopItem title="Top Pages" items={data?.topPages} icon={ChartBarIcon} />
        <TopItem
          title="Top Referrers"
          items={data?.topReferrers}
          icon={EyeIcon}
        />
        {/* TODO: Implement IP geolocation to populate countryDistribution data */}
        {/* <TopItem
                    title="Top Countries"
