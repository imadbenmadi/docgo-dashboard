...
const DataSeederPanel = ({ onDataSeeded }) => {
  ...
  const handleSeedData = async (count = 150) => {
    const result = await Swal.fire({
      title: "Seed Login Data?",
      text: `Are you sure you want to seed ${count} login records? This will clear existing data.`,
      icon: "warning",
  ...
  const handleClearData = async () => {
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to clear ALL login records? This action cannot be undone!",
      icon: "error",
      showCancelButton: true,
    ...
    try {
      ...
      toast.success(`Cleared ${response.data.deletedCount} login records`);
    ...
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6 mb-6">
      ...
      {seedingInfo && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="bg-white p-3 rounded-lg border">
            ...
            >
              {seedingInfo.currentStats.hasData ? "Available" : "Empty"}
            </div>
          </div>
          <div className="bg-white p-3 rounded-lg border">
            <div className="text-sm text-gray-600">Date Range</div>
            <div className="text-sm font-medium text-gray-700">
              {seedingInfo.currentStats.hasData ? "Last 30 days" : "No data"}
            </div>
          </div>
        </div>
      )}
      ...
      {showInfo && seedingInfo && (
        <div className="bg-white p-4 rounded-lg border mb-4">
          <h4 className="font-medium text-gray-900 mb-2">Seeding Features:</h4>
          ...
          <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-yellow-800">
                <strong>Note:</strong> {seedingInfo.seedingOptions.description}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-3 text-xs text-gray-500">
        💡 Tip: Start with 150 records for testing, use 500+ for performance
        testing
      </div>
    </div>
  );
