...
export const adminUsersAPI = {
  ...
  getUserDetails: async (userId) => {
    try {
      const response = await axios.get(`${ADMIN_BASE_URL}/users/${userId}`);
  ...
  getAllCoursesForAssignment: async () => {
    try {
      const response = await axios.get(`${ADMIN_BASE_URL}/courses/available`);
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
    }
  ...
  getAllProgramsForAssignment: async () => {
    try {
      const response = await axios.get(`${ADMIN_BASE_URL}/programs/available`);
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
    }
  ...
  /**
   * Sync enrollment counts — recalculates Users_count for all courses and
   * programs from the actual enrollment tables. Call once to fix stale data.
   */
  syncEnrollmentCounts: async () => {
    try {
      const response = await axios.post(
        `${ADMIN_BASE_URL}/sync-enrollment-counts`,
      );
      return response.data;
    } catch (error) {
      throw error.response?.data || error.message;
