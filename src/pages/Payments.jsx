...
const AdminPaymentDashboard = () => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (filters.paymentMethod === "ccp" || filters.paymentMethod === "") {
      ...
      if (response.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Erreur",
          text: response.message || "Impossible de charger les paiements",
        });
      }
    } catch (error) {
      Swal.fire({
      ...
      try {
        ...
        if (response.success) {
          ...
          Swal.fire({
            icon: "error",
            title: "Erreur",
            text: response.message || "Échec de l'approbation du paiement",
          });
        }
      } catch (error) {
        Swal.fire({
  ...
  const handleDeletePayment = async (payment) => {
    const result = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        if (!reason || reason.trim() === "") {
          Swal.showValidationMessage("Le motif de suppression est requis");
          return false;
        }
        return reason.trim();
      },
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600 mt-1">
                  {statistics.overview?.completedPayments || 0}
                </p>
              </div>
                  ...
                  <tr key={payment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-900">CCP</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {(payment.paymentMethod === "ccp" ||
                          payment.CCP_number ||
                          payment.transactionId?.startsWith("CCP_")) && (
                          <button
              ...
              <div className="mb-4 p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border-2 border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-3 rounded-lg shadow-sm">
                    ...
                    >
                      {selectedPayment.User?.phoneNumber || "Not provided"}
                    </a>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 font-mono">
                      {selectedPayment.CCP_number ||
                        selectedPayment.CCPPayment?.CCP_number ||
                        "N/A"}
                    </p>
                  </div>
                  <div>
                ...
                selectedPayment.CCPPayment?.transactionId ? (
                  <>
                    ...
                    <p className="text-xs text-gray-600 mt-3 text-center bg-blue-50 p-2 rounded">
                      💡 <strong>Tip:</strong> Click image to view full size, or
                      use the buttons above to view/download
                    </p>
                  </>
                ) : (
