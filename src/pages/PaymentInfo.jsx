  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const configData = {
        ...
        default_currency: "DZD",
        supported_currencies: ["DZD"],
        payment_description_template: formData.payment_description_template,
      ...
      if (paymentConfig?.id) {
        ...
        response = await PaymentConfigAPI.createPaymentConfig(configData);
      }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          ...
          options de paiement CCP pour votre plateforme.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        {Object.entries(paymentMethods).map(([method, info]) => {
          ...
          const isConfigured = !!(
            paymentConfig?.ccp_account_number || paymentConfig?.ccp_account_name
          );
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        name="is_ccp_enabled"
                        checked={formData.is_ccp_enabled}
                      ...
                      {viewMode ? (
                        <div className="p-3 bg-gray-50 rounded-md text-sm">
                          {formData.ccp_instructions ? (
                            <RichTextDisplay
                              content={formData.ccp_instructions}
                              textClassName="prose prose-sm max-w-none"
                            />
                          ) : (
                            <span>No instructions set</span>
                          )}
                        </div>
                      ) : (
                        <RichTextEditor
                          value={formData.ccp_instructions}
                          onChange={(value) =>
                            handleRichTextChange("ccp_instructions", value)
                          }
