                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        ...
                        Cours ({userDetails?.enrollments?.courses?.length || 0})
                      </h3>
                      <button
                        onClick={() => handleAssignCourse(selectedUser)}
                    ...
                    {userDetails?.enrollments?.courses?.length > 0 ? (
                      <div className="space-y-2">
                        {userDetails.enrollments.courses.map((course) => (
                          <div
                            key={course.id}
                          ...
                          >
                            ...
                            <button
                              onClick={() =>
                                handleRemoveFromCourse(course.courseId)
                              }
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        ...
                        Programmes (
                        {userDetails?.enrollments?.programs?.length || 0})
                      </h3>
                      <button
                        onClick={() => handleAssignProgram(selectedUser)}
                        className="px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm flex items-center gap-1"
                      >
                        <Edit2 className="w-3 h-3" />
                        Assigner
                    ...
                    {userDetails?.enrollments?.programs?.length > 0 ? (
                      <div className="space-y-2">
                        {userDetails.enrollments.programs.map((program) => (
                          <div
                            key={program.id}
                            className="flex items-center justify-between bg-gray-50 p-3 rounded-lg"
                          >
                            ...
                            <button
                              onClick={() =>
                                handleRemoveFromProgram(program.programId)
                              }
                              className="px-2 py-1 bg-red-100 text-red-600 rounded hover:bg-red-200 text-sm"
                            >
                              Retirer
                      ...
                      <p className="text-gray-600">Aucun programme assigné</p>
                    )}
                  </div>
              ...
              {courses.length === 0 ? (
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {(Array.isArray(courses) ? courses : []).map((course) => (
                    <button
                      key={course.id}
                      onClick={() => confirmAssignCourse(course.id)}
              ...
              {programs.length === 0 ? (
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {(Array.isArray(programs) ? programs : []).map((program) => (
                    <button
                      key={program.id}
                      onClick={() => confirmAssignProgram(program.id)}
                      className="w-full text-left p-3 bg-gray-50 hover:bg-blue-50 rounded-lg border border-gray-200 hover:border-blue-400 transition-colors"
                    >
                      <p className="font-semibold text-gray-800">
                        {program.title}
