...
const CourseApplications = () => {
  ...
  const fetchApplications = async () => {
    setLoading(true);
    try {
      const res = await ApplicationsAPI.getCourseApplications({
  ...
  const getPaymentForApplication = (app) => {
    ...
    return (
      ccpPayments.find(
        (p) =>
          p.userId === app.UserId &&
          p.itemId === app.CourseId &&
          p.itemType === "course",
      ) || null
    );
  };

  // Returns ALL payments for a given application (for full history display)
  const getPaymentsForApplication = (app) => {
    return ccpPayments.filter(
      (p) =>
        p.itemType === "course" &&
        p.itemId === app.CourseId &&
        p.userId === app.UserId,
  ...
  const freeApplications = applications.filter((a) => a.paymentType === "free");
  const paidApplications = applications.filter((a) => a.paymentType !== "free");
  ...
  const handleApprove = async (app) => {
    const result = await Swal.fire({
      ...
      html: `<p>Approuver la candidature de <b>${app.User?.firstName} ${app.User?.lastName}</b> pour <b>${app.Course?.title}</b> ?</p>`,
    ...
    setProcessing(true);
    const res = await ApplicationsAPI.updateCourseApplicationStatus(app.id, {
      status: "approved",
  ...
  const handleApprovePaidPayment = async (app) => {
    ...
    const result = await Swal.fire({
      ...
      html: `<p>Approuver le paiement CCP de <b>${app.User?.firstName} ${app.User?.lastName}</b> pour le cours <b>${app.Course?.title}</b> ?</p>`,
      showCancelButton: true,
    ...
    setProcessing(true);
    const res = await ApplicationsAPI.approveCoursePayment(payment.id);
    setProcessing(false);
    if (res.success) {
  ...
  const handleRejectPaidPayment = async (app) => {
    ...
    setProcessing(true);
    const res = await ApplicationsAPI.rejectCoursePayment(payment.id, reason);
    setProcessing(false);
    if (res.success) {
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage free enrollments and CCP payments for courses
          </p>
        </div>
        <button
          onClick={() => {
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredList.map((app) => {
                  const payment =
                    activeTab === "paid" ? getPaymentForApplication(app) : null;
                  const imgSrc = payment ? getImageSrc(payment) : null;
                  return (
                    <motion.tr
                      key={app.id}
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900 max-w-[160px] truncate">
                          {app.Course?.title || `Cours #${app.CourseId}`}
                        </p>
                        <p className="text-xs text-gray-500">
                          {PAYMENT_TYPE_LABELS[app.paymentType] ||
                            app.paymentType}
                        </p>
                      </td>
                      {activeTab === "paid" && (
                            ...
                            {payment ? (
                              <div>
                                <p className="text-xs font-mono text-gray-600">
                                  N°:{" "}
                                  {payment.CCP_number || app.CCP_number || "—"}
                                </p>
                                <p className="text-xs text-gray-500">
                                  {payment.amount
                                    ? `${payment.amount} DZD`
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-4">
                ...
                <div className="bg-indigo-50 rounded-xl p-4">
                  ...
                  <p className="text-sm text-indigo-600">
                    {PAYMENT_TYPE_LABELS[selectedApplication.paymentType] ||
                      selectedApplication.paymentType}
                  </p>
                  {selectedApplication.Course?.price && (
                    <p className="text-sm text-indigo-600">
                      Prix: {selectedApplication.Course.price} DZD
                    </p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[selectedApplication.status] || "bg-gray-100 text-gray-700"}`}
                    >
                ...
                {selectedApplication.paymentType !== "free" &&
                  (() => {
                    const allPayments =
                      getPaymentsForApplication(selectedApplication);
                    ...
                    return (
                      <div className="bg-yellow-50 rounded-xl p-4 space-y-3">
                        ...
                        {allPayments.map((payment) => {
                          const imgSrc = getImageSrc(payment);
                          const statusColor =
                            payment.status === "approved"
                          ...
                          return (
                            <div
                              key={payment.id}
                              className="bg-white rounded-lg p-3 border border-yellow-200 flex gap-3 items-start"
                            >
                              {imgSrc ? (
                                <button
                                  onClick={() => openImageModal(imgSrc)}
                                ...
                                >
                                  <img
                                    src={imgSrc}
                                    alt="Receipt"
                                    className="w-14 h-14 object-cover rounded border border-gray-200 hover:opacity-90 transition-opacity"
                                  />
                                </button>
                              ) : (
                              ...
                              <div className="flex-1 text-sm">
                                <div className="flex items-center gap-2 mb-1">
                                  <span
                                    className={`px-1.5 py-0.5 text-xs font-medium rounded ${statusColor}`}
                                  >
                                    {payment.status}
                                  </span>
                  ...
                  <button
                    onClick={() => {
                      ...
                      selectedApplication.paymentType === "free"
                        ...
                        : handleApprovePaidPayment(selectedApplication);
                    }}
                  ...
                  <button
                    onClick={() => {
                      ...
                      selectedApplication.paymentType === "free"
                        ...
                        : handleRejectPaidPayment(selectedApplication);
                    }}
