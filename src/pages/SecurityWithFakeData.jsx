  ...
  const fetchLogins = useCallback(async () => {
    ...
    try {
      if (useFakeData) {
        ...
        const response = await axios.get(`/Admin/logins?${params.toString()}`);
  ...
  const handleBulkThreatDismissal = async () => {
    ...
    try {
      if (useFakeData) {
        ...
        toast.success(
          `${selectedLogins.length} threats marked as handled (fake)`,
        );
      } else {
        ...
        <div className="max-w-md mx-auto text-center p-8">
          <div className="mb-6">
            ...
            <p className="text-gray-600">
              No login data available yet. Login attempts will appear here when
              they occur.
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Your system is secure. We'll monitor for any suspicious login
              attempts.
            </p>
            <button
              onClick={fetchLogins}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold text-gray-900">
              Security Dashboard{" "}
              {useFakeData && (
                <span className="text-sm text-orange-600">(Fake Data)</span>
              )}
            </h1>
          </div>
          <div className="flex items-center gap-3">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {loginData.stats.successRate || 0}% success rate
                  </p>
                </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-red-500">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {loginData.stats.threatRate || 0}% threat rate
                  </p>
                </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between">
              <p className="text-sm text-blue-800">
                {selectedLogins.length} login attempt(s) selected
              </p>
              <div className="flex items-center gap-3">
                <button
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <button
              onClick={() => {
                const allIds = loginData.logins?.map((login) => login.id) || [];
                setSelectedLogins(
                  selectedLogins.length === allIds.length ? [] : allIds,
                );
              }}
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <input
                      ...
                      onChange={() => {
                        const allIds =
                          loginData.logins?.map((login) => login.id) || [];
                        setSelectedLogins(
                          selectedLogins.length === allIds.length ? [] : allIds,
                        );
                      }}
                ...
                {loading ? (
                  ...
                  loginData.logins?.map((login) => (
                    <tr
                      key={login.id}
                      className={
                        login.isThreat ? "bg-red-50" : "hover:bg-gray-50"
                      }
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedLogins([...selectedLogins, login.id]);
                            } else {
                              setSelectedLogins(
                                selectedLogins.filter((id) => id !== login.id),
                              );
                            }
                          }}
                          className="rounded"
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {login.isThreat ? (
                          <div>
                            ...
                            {login.threatReasons &&
                              login.threatReasons.length > 0 && (
                                <div className="text-xs text-gray-500 mt-1">
                                  {login.threatReasons.slice(0, 2).join(", ")}
                                  {login.threatReasons.length > 2 && " ..."}
                                </div>
                              )}
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {login.isThreat && (
                          <button
                            onClick={() => handleThreatDismissal(login.id)}
          ...
          {/* Pagination */}
          {loginData.pagination && loginData.pagination.totalPages > 1 && (
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 flex justify-between sm:hidden">
                  <button
                    ...
                    disabled={!loginData.pagination.hasPrev}
                  ...
                  <button
                    ...
                    disabled={!loginData.pagination.hasNext}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      ...
                      <span className="font-medium">
                        {loginData.pagination.currentPage}
                      </span>{" "}
                      of{" "}
                      <span className="font-medium">
                        {loginData.pagination.totalPages}
                      </span>{" "}
                      ({loginData.pagination.totalCount} total)
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-3 py-2 text-sm text-gray-700">
                      Page {filters.page} of {loginData.pagination.totalPages}
                    </span>
                    <button
                      onClick={() =>
                        setFilters({
                    ...
                    <button
                      onClick={() =>
                        setFilters({
                          ...filters,
                          page: loginData.pagination.totalPages,
                        })
                      }
                      disabled={
                        filters.page === loginData.pagination.totalPages
                      }
                      className="px-3 py-2 border border-gray-300 text-sm rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50"
                    >
                      Last
