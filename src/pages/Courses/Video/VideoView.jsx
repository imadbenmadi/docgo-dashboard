...
const VideoView = () => {
  ...
  const fetchVideoDetails = useCallback(async () => {
    try {
      setLoading(true);
      const response = await coursesAPI.getVideoDetails(courseId, videoId);
  ...
  // Fetch a signed stream token once the video is loaded
  useEffect(() => {
    if (!video) return;
    const rawPath = video.VideoUrl || video.video;
    if (!rawPath) return;
    ...
    coursesAPI
      .getAdminStreamToken(courseId, "video", filename)
      .then(({ streamPath }) => setStreamUrl(`${API_URL}${streamPath}`))
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <div className="mt-4">
          ...
          {course && (
            <p className="text-lg text-gray-600 mt-2">Course: {course.Title}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div className="lg:col-span-2">
          {/* Video Player */}
          {streamLoading && (
            <div className="flex items-center justify-center aspect-video bg-black rounded-lg mb-6 text-white gap-3">
              <svg
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 text-gray-600">
                  {formatDuration(parseInt(video.Duration) || 0)}
                </span>
              </div>
              <div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {formatFileSize(pdf.size)} • Order #{index + 1}
                            </p>
                          </div>
                        </div>
                        <button
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-purple-50 rounded-lg p-6 text-center">
                  ...
                  <div className="text-sm text-purple-600">Avg Quiz Score</div>
                </div>
              </div>
            </div>
          )}
