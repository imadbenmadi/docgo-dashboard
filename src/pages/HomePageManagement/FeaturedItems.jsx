  ...
  return (
    <div
      className={`flex items-center gap-3 p-4 rounded-2xl border transition-all ${
        featured ? "border-amber-200 bg-amber-50" : "border-gray-100 bg-white"
      }`}
    >
      {/* Thumbnail */}
      <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0">
        <ImageWithFallback
          type={isCourse ? "course" : "program"}
          src={thumb || null}
          alt={title}
...
const FeaturedItems = () => {
  ...
  const handleToggleCourse = async (id, newState) => {
    ...
    if (res.success) {
      setCourses((prev) =>
        prev.map((c) => (c.id === id ? { ...c, isFeatured: newState } : c)),
      );
      toast.success(
        newState
  ...
  const handleToggleProgram = async (id, newState) => {
    ...
    if (res.success) {
      setPrograms((prev) =>
        prev.map((p) => (p.id === id ? { ...p, isFeatured: newState } : p)),
      );
      toast.success(
        newState
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">
            Sélectionnez les cours et programmes affichés sur la page
            d&apos;accueil
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        {TABS.map(({ id, label, icon: Icon }) => {
          const count = (id === "courses" ? courses : programs).filter(
            (i) => i.isFeatured,
          ).length;
          return (
            <button
      ...
      <p className="text-xs text-gray-500 mb-3">
        {featuredCount} élément{featuredCount !== 1 ? "s" : ""} en vedette ·{" "}
        ...
        <div className="grid gap-3">
          {filtered.map((item) => (
            <ItemCard
              ...
              item={item}
              type={tab}
              busy={!!busy[`${tab === "courses" ? "c" : "p"}-${item.id}`]}
              onToggle={
                tab === "courses" ? handleToggleCourse : handleToggleProgram
              }
            />
          ))}
        </div>
