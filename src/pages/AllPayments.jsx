...
// Enhanced PaymentProofModal with Accept/Reject functionality
      ...
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4">
        ...
        <div className="flex justify-center gap-4">
          ...
          <button
            ...
            disabled={isProcessing}
            className="flex items-center gap-2 px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
...
const mockAPI = {
  async getUsers(page = 1, pageSize = 10, filters = {}) {
    ...
    let filteredUsers = allUsers.filter(
      (user) =>
        user.paymentStatus === "Pending" || user.paymentStatus === "Failed"
    );
  ...
  async getApplications(page = 1, pageSize = 10, filters = {}) {
    ...
    let filteredApps = allApplications.filter(
      (app) => app.paymentStatus === "Pending" || app.paymentStatus === "Failed"
    );
...
const AllPayments = () => {
  ...
  const handleViewPaymentProof = (imageUrl, userName, userId) => {
  ...
  const handleApprovePayment = async (userId) => {
    try {
      const type = activeTab === "Apprendre des cours" ? "user" : "application";
  ...
  return (
    <div className="w-full bg-gray-50 p-4">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Review and approve pending payments</p>
      </div>
